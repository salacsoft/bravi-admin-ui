    ...
    <div class="h-screen flex flex-col justify-between bg-gray-100 text-gray-900">
        <div class="h-5/6 md:h-full flex flex-col md:flex-row justify-center">
            ...
            <div class="md:w-2/5 md:h-full md:flex md:items-center md:justify-center ">
                <form class="w-full flex flex-col items-center px-6" @submit.prevent="login">
                    ...
                    <h2 class="font-semibold text-2xl lg:text-4xl mb-10 ">{{productName}}</h2>
                    ...
                    <button 
                        class="px-4 py-2 w-full  max-w-sm tracking-wider rounded-xl bg-gradient-to-r from-yellow-400 via-pink-500 to-red-500"
                    >
                        Login
                    </button>
    ...
    setup () {
        ...
        const login = () => {
            API.post("/login", credential).then(response => {
                ...
                store.dispatch("setUserDetails",  {email, full_name, uuid, user_uuid, user_id});
                router.push("/");
